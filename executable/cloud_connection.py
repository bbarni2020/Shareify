...
class ShareifyLocalClient:
    ...
    def setup_handlers(self):
        ...
        def on_auth_success(data):
            ...
            config_data = {'user_id': self.user_id, 'auth_token': self.auth_token, 'username': data['username'], 'server_id': self.server_id, 'server_name': self.server_name, 'cloud_url': self.cloud_url, 'enabled': self.enabled, 'last_authentication': time.time()}
        ...
        def on_auth_failed(data):
            ...
            if self.auth_token and self.username and self.password:
                ...
                self.sio.emit('authenticate_user', {'username': self.username, 'password': self.password})
            else:
        ...
        def on_registration_success(data):
            if 'server_id' in data:
                ...
                config_data.update({'server_registered': True, 'registration_timestamp': time.time(), 'server_id': self.server_id, 'server_name': self.server_name, 'user_id': self.user_id, 'auth_token': self.auth_token, 'username': self.username, 'cloud_url': self.cloud_url, 'enabled': self.enabled})
                save_cloud_config(config_data)
                ...
                self.auth_token = data.get('auth_token')
                self.authenticated = True
                config_data = {'user_id': self.user_id, 'auth_token': self.auth_token, 'username': data['username'], 'password': self.password, 'server_id': self.server_id, 'server_name': self.server_name, 'cloud_url': self.cloud_url, 'enabled': self.enabled, 'user_registered': True, 'user_registration_timestamp': time.time()}
                save_cloud_config(config_data)
        ...
        def on_execute_command(data):
            ...
            try:
                ...
                if self.sio.connected:
                    try:
                        self.sio.emit('command_response', {'command_id': command_id, 'response': {'error': str(e)}})
    ...
    def authenticate_user(self):
        if self.user_id and self.auth_token:
            ...
            self.sio.emit('authenticate_user', {'user_id': self.user_id, 'auth_token': self.auth_token})
            ...
            self.sio.emit('register_user', {'username': new_username, 'password': new_password})
    ...
    def register_server(self):
        if not self.authenticated:
            ...
            return
        self.sio.emit('register_server', {'server_id': self.server_id, 'name': self.server_name, 'user_id': self.user_id, 'auth_token': self.auth_token})
    ...
    def get_server_info(self):
        return {'server_id': self.server_id, 'server_name': self.server_name, 'cloud_url': self.cloud_url, 'connected': self.connected, 'enabled': self.enabled, 'heartbeat_interval': self.heartbeat_interval, 'command_timeout': self.command_timeout}

    def handle_shareify_command(self, command):
        parts = command.split(' ', 1)
        action = parts[0]
        if action == 'status':
            return {'server_id': self.server_id, 'server_name': self.server_name, 'connected': self.connected, 'enabled': self.enabled, 'uptime': time.time(), 'platform': sys.platform, 'heartbeat_interval': self.heartbeat_interval, 'command_timeout': self.command_timeout}
        elif action == 'info':
            ...
            while self.connected and self.enabled:
                try:
                    if self.authenticated:
                        self.sio.emit('ping', {'server_id': self.server_id, 'user_id': self.user_id, 'timestamp': time.time()})
    ...
    def connect(self):
        ...
        for attempt in range(max_retries):
            try:
                ...
                self.sio.connect(self.cloud_url, wait_timeout=15, socketio_path='/socket.io/', headers={'User-Agent': f'Shareify-Client/{self.server_id}'})
    ...
    def execute_api_request(self, command_id, url, method='GET', body=None, shareify_jwt=None):
        try:
            ...
            allowed_endpoints = ['/resources', 'resources', '/is_up', 'is_up', '/user/get_self', 'user/get_self', '/user/login', 'user/login', '/get_logs', '/finder', '/get_file']
            if not any((url == ep or url.startswith(ep) for ep in allowed_endpoints)):
                if self.sio.connected:
                    try:
                        self.sio.emit('command_response', {'command_id': command_id, 'response': {'error': 'Not allowed (security reasons) to access this endpoint.'}})
                    except Exception as emit_error:
            ...
            if self.sio.connected:
                try:
                    self.sio.emit('command_response', {'command_id': command_id, 'response': {'error': 'API request timeout'}})
                except Exception as emit_error:
            ...
            if self.sio.connected:
                try:
                    self.sio.emit('command_response', {'command_id': command_id, 'response': {'error': 'Failed to connect to local API server'}})
                except Exception as emit_error:
    ...
    def send_standard_response(self, command_id, response_data):
        if self.sio.connected:
            try:
                self.sio.emit('command_response', {'command_id': command_id, 'response': response_data})
    ...
    def send_file_via_storage(self, command_id, response_data):
        try:
            ...
            storage_data = {'content': content, 'content_type': content_type, 'filename': filename, 'server_id': self.server_id, 'user_id': self.user_id, 'auth_token': self.auth_token}
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    store_response = requests.post(f'{self.cloud_url}/cloud/file/store', json=storage_data, timeout=dynamic_timeout, headers={'Connection': 'close'})
                    if store_response.status_code == 200:
                        ...
                        if storage_result.get('success'):
                            ...
                            file_response = {'status': response_data.get('status', 'File stored successfully'), 'type': 'file_storage', 'file_id': file_id, 'password': password, 'original_type': content_type, 'filename': filename, 'file_size': content_size}
        ...
        try:
            ...
            if content_type == 'text' and isinstance(content, str):
                ...
                if len(content.encode('utf-8')) > max_fallback_size:
                    ...
                    fallback_response = {'status': f'File too large for cloud storage, showing first {len(truncated_content)} characters', 'content': truncated_content + f'\n\n[TRUNCATED - Original file was {content_size} bytes]', 'type': 'text', 'filename': filename, 'truncated': True, 'original_size': content_size, 'storage_error': str(storage_error)}
                    print(f'Sending truncated content fallback for {filename}')
                    self.send_standard_response(command_id, fallback_response)
                    return
            error_response = {'error': f'File storage failed and content too large for direct transfer', 'filename': filename, 'file_size': content_size, 'content_type': content_type, 'storage_error': str(storage_error), 'suggestion': 'Try downloading smaller files or check network connection'}
    ...
    def send_error_response(self, command_id, error_message):
        if self.sio.connected:
            try:
                self.sio.emit('command_response', {'command_id': command_id, 'response': {'error': error_message, 'status': 'error'}})
