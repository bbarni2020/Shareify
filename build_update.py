...
if __name__ == "__main__":
    try:
        ...
        print(f"Error importing update module: {e}")
        ...
        sys.exit(1)
'''
        entry_script_path = self.script_dir / 'update_entry.py'
...
for py_file in Path(executable_dir).glob("*.py"):
    if py_file.name not in ['build_update.py', 'update_entry.py']:
...
'''
        spec_path = self.script_dir / 'update.spec'
    ...
    def cleanup_temp_files(self):
        temp_files = ['update_entry.py', 'update.spec']
    ...
    def build(self):
        self.log('Starting Update executable build...')
        self.log('=' * 50)
        ...
        update_dist = self.dist_dir / 'update'
        if os.name == 'nt':
            exe_path = update_dist / 'update.exe'
        else:
            exe_path = update_dist / 'update'
        
        if exe_path.exists():
            self.log(f'✓ Executable: {exe_path}')
        
        self.log(f'✓ Distribution folder: {update_dist}')
        self.log('✓ Ready to zip and deploy!')
        
        return True

def main():
    builder = UpdateExecutableBuilder()
    if len(sys.argv) > 1:
        if sys.argv[1] == '--clean':
